...
APPS_DATASETS=("homarr" "prowlarr" "radarr" "sonarr" "lidarr" "lazylibrarian" "jellyfin" "jellyseerr" "bazarr" "qbittorrent")
MEDIA_SUBDIRECTORIES=("movies" "tv" "music" "books")
DOCKER_COMPOSE_PATH="/mnt/$POOLNAME/docker"
QBITTORRENT_WIREGUARD_DIR="/mnt/$POOLNAME/apps/qbittorrent/wireguard"
...
# Create the "apps" dataset (parent)
create_dataset "apps"

# Create the apps datasets
for dataset in "${APPS_DATASETS[@]}"; do
    create_dataset "apps/$dataset"
done
...
services:
  homarr:
    ...
    environment:
      - SECRET_ENCRYPTION_KEY=e3d89486e9985f8cb631f28b0a2beb4296fffe27e9056bede8c2d13f343dcd34
    ports:
      - '7575:7575'
  ...
  prowlarr:
    ...
    networks:
      - media_network
    volumes:
      - /mnt/$POOLNAME/apps/prowlarr:/config
  ...
  radarr:
    ...
    networks:
      - media_network
    volumes:
      - /mnt/$POOLNAME/apps/radarr:/config
      - /mnt/$POOLNAME/media:/media
  ...
  sonarr:
    ...
    networks:
      - media_network
    volumes:
      - /mnt/$POOLNAME/apps/sonarr:/config
      - /mnt/$POOLNAME/media:/media
  ...
  lidarr:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
     - /mnt/$POOLNAME/apps/lidarr:/config
      - /mnt/$POOLNAME/media/music:/music #optional
      - /mnt/$POOLNAME/downloads:/downloads #optional
    ports:
      - 8686:8686
    restart: unless-stopped
  ...
  lazylibrarian:
    ...
    volumes:
      - /mnt/$POOLNAME/apps/lazylibrarian/data:/config
      - /mnt/$POOLNAME/downloads:/downloads
      - /mnt/$POOLNAME/media/books:/books #optional
    ports:
      - 5299:5299
    restart: unless-stopped
  ...
  jellyfin:
    ...
    networks:
      - media_network
    volumes:
      - /mnt/$POOLNAME/apps/jellyfin:/config
      - /mnt/$POOLNAME/media:/media
  ...
  jellyseerr:
    ...
    user: "568:568"
    volumes:
      - /mnt/$POOLNAME/apps/jellyseerr:/app/config
  ...
  bazarr:
    ...
    networks:
      - media_network
    volumes:
      - /mnt/$POOLNAME/apps/bazarr:/config
      - /mnt/$POOLNAME/media:/media
  ...
  qbittorrent:
    ...
    volumes:
      - /mnt/$POOLNAME/apps/qbittorrent:/config
      - /mnt/$POOLNAME/media:/media
...
if [[ "$LAUNCH_CONTAINERS" =~ ^[Yy]es$ ]]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        QBITTORRENT_CONF_FILE="/mnt/$POOLNAME/apps/qbittorrent/config/qBittorrent.conf"
        ...
        if grep -q "\[BitTorrent\]" "$QBITTORRENT_CONF_FILE"; then
            ...
            if grep -q "Session\\DefaultSavePath=" "$QBITTORRENT_CONF_FILE"; then
                ...
                sed -i "/\[BitTorrent\]/,/^\[/ s|Session\\DefaultSavePath=.*|Session\\DefaultSavePath=/downloads|" "$QBITTORRENT_CONF_FILE"
            else
            ...
            echo -e "\n[BitTorrent]\nSession\\DefaultSavePath=/downloads" >> "$QBITTORRENT_CONF_FILE"
        fi

        echo "qBittorrent default save path set to /downloads in $QBITTORRENT_CONF_FILE"
        ...
        if [[ "$CONFIGURE_RECYCLARR" =~ ^[Yy]es$ ]]; then
            ...
            RADARR_CONFIG_FILE="/mnt/$POOLNAME/apps/radarr/config.xml"
            SONARR_CONFIG_FILE="/mnt/$POOLNAME/apps/sonarr/config.xml"
            ...
            RECYCLARR_YML="/mnt/$POOLNAME/apps/recyclarr/recyclarr.yml"
